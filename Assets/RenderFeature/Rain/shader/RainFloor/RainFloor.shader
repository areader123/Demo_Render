Shader "Unlit/RainFloor"
{
    Properties
    {
        ...
        [Toggle(_WATER_RIPPLE)] _WATER_RIPPLE ("Ripple", float) = 0
        ...
        [Toggle(_WATER_FLOW)] _WATER_FLOW ("Water Flow", float) = 0
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            #include"Assets/RenderFeature/ShaderLibs/WeaterCore.hlsl"
            ...
            struct Varyings
            {
                ...
                float4 color : COLOR;
                float2 uv : TEXCOORD0;
                //有LIGHTMAP_ON 则申明 lightmap 没有则申明vertexSH
            ...
            Varyings vert (Attributes IN)
            {
                ...
                //此处为计算逐顶点计算额外光源 VertexLighting() 与宏 _ADDITIONAL_LIGHTS_VERTEX 搭配使用
                ...
                //与 #pragma multi_compile _ LIGHTMAP_ON 搭配使用
                ...
                #ifdef REQUIRES_VERTEX_SHADOW_COORD_INTERPOLATOR
                    //此处将世界空间转换为阴影空间
                    //与 _MAIN_LIGHT_SHADOWS_CASCADE 搭配使用
                    OUT.shadowCoord = GetShadowCoord(positionInputs);
                #endif
                return OUT;
            }  

            InputData InitializeInputData(Varyings IN, half3 normalTS)//填充计算数据
            {
                ...
                #if defined(REQUIRES_VERTEX_SHADOW_COORD_INTERPOLATOR)//当前使用的时这个分支
            ...
            SurfaceData InitializeSurfaceData(Varyings IN)//填充表面信息
            {
            ...
            half4 frag (Varyings IN) : SV_Target
            {
                ...
                // return float4(wetlandData.normalTS,1);
                ...
                //return float4(wetlandData.normalTS,1);
                //return float4(wetlandData.albedo,1);
