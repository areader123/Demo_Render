Shader "Unlit/RainPlane"
{
    Properties
    {
        ...
        _FarPlane("Far Plane",float) = 0

        //[Header("Water")]
        _FlowMap("FlowMap",2D) = "white" {}
        _Tilling("Flow Tilling",float) = 1
        _FlowSpeed("Flow Speed",float) = 2
        _FlowStrenght("Flow Strenght",float) =1
        _WaterBaseMap("Water Normal Texture", 2D) = "bump" { }
        _WaterBumpScale ("Water Normal Scale",float) = 1
        _BlendContrast("Height Lerp BlendContrast",Range(0,1)) = 0.1
    ...
    SubShader
    {
        ...
        TEXTURE2D(_WaterBaseMap);
        SAMPLER(sampler_WaterBaseMap);
        TEXTURE2D(_WaterBumpScale);
        SAMPLER(sampler_WaterBumpScale);
        ...
        Pass
        {
            ...
            SurfaceData InitializeSurfaceData(Varyings IN)
            {
                ...
                half4 water = FlowMapNode(TEXTURE2D_ARGS(_FlowMap,sampler_FlowMap),TEXTURE2D_ARGS(_FlowMap,sampler_FlowMap),IN.uv,_Tilling,_FlowSpeed,_FlowStrenght);
                half height = SAMPLE_TEXTURE2D(_ParallaxMap,sampler_ParallaxMap,IN.uv);
                half lerpWithHeight = HeigthLerp(height,IN.color.r,_BlendContrast);
                ...
                surfaceData.normalTS = lerp(surfaceData.normalTS,water.rgb,lerpWithHeight);
                #ifdef _EMISSION
            ...
            half4 frag(Varyings IN) : SV_Target
            {
                ...
                float3 viewDirTS = GetViewDirectionTangentSpace(IN.tangentWS, IN.normalWS, viewDirWS);
                    ...
                    ddx(IN.uv), ddy(IN.uv), viewDirTS, numSteps, _ParallaxAmplitude, _FarPlane);
                    IN.uv = offset;
                #else
                    //此处是unity提供的视差贴图采样
